...
def main() -> None:
    ...
    p.add_argument("--input-dir", type=Path, default=Path("data_dir", "cycic3b-sample"))
    p.add_argument("--output-dir", type=Path, default=Path("task_data", "cycic3b-sample-t5"))
    ...
    with (args.output_dir / f"{out_split}.source").open("w") as file:
    ...
    with (args.output_dir / f"{out_split}.target").open("w") as file:
        for question, label in zip(new_questions, new_labels):
            if len(question["answer_options"]) == 5:
                target = "%s" % str(question["answer_options"][int(label) - 1])
            elif len(question["answer_options"]) == 2:
