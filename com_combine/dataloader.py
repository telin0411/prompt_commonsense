...
class Com2SenseDataset(BaseDataset):
    ...
    def _preprocess(self, data_dir):
        ...
        # Map category labels
        domain2int = {'physical': 0, 'time': 1, 'social': 2}
        scenario2int = {'comparison': 0, 'causal': 1}
        numeracy2int = {'False': 0, 'True': 1}
        data = []
        if self.split == 'train' or 'dev':
            df['domain'] = df['domain'].apply(lambda l: domain2int[l])
            df['scenario'] = df['scenario'].apply(lambda l: scenario2int[l])
            df['numeracy'] = df['numeracy'].apply(lambda l: numeracy2int[l])
            ...
            for pair in raw_data:
                label_num = 3*(pair['scenario'] + 2*pair['numeracy']) + pair['domain']
                sample_1 = dict(_id=pair['_id'], text=pair['sent_1'], label=label_num)
                sample_2 = dict(_id=pair['_id'], text=pair['sent_2'], label=label_num)
