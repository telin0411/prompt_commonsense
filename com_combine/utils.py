...
def confusion_matrix(gt, pred):
    ...
    return confusionMatrix, (pt, pf, nt, nf)
...
def compute_eval_metrics(model, dataloader, device, size, tokenizer, text2text=False, is_pairwise=False, is_test=False,
    ...
    confusionMatrix, cls_index = confusion_matrix(ground_truth, predicted)

    # to see which sentence the model fail to classify
    fail_sentence = np.array(input_decoded)
    nt_sentence = fail_sentence[cls_index[1]]
    pf_sentence = fail_sentence[cls_index[2]]
    print("=========== negative true  ==========")
    print(nt_sentence)
...
def category_acc(category_pred_list, category_gt_list):
    """
    :param category_pred_list: [[0, 1, ..., 2], [1, 0, ..., 1], [0, 0,..., 1]]
    :param   category_gt_list: [[0, 1, ..., 0], [0, 0, ..., 1], [0, 0,..., 1]]
    :return: accuracy if the sentence is right in all categories
    """
    assert len(category_gt_list) == len(category_pred_list) and len(category_gt_list) == 3, "Length of prediction and " \
                                                                                            "ground true should be " \
                                                                                            "the same "
    accuracy_list = []
    for i in range(3):
        accuracy_list.append(np.array(category_pred_list[i]) == np.array(category_gt_list[i]))

    accuracy_array = np.logical_and(accuracy_list[0], accuracy_list[1], accuracy_list[2])
    zero_one = np.bincount(accuracy_array)
    return zero_one[1] / (zero_one[0] + zero_one[1])
