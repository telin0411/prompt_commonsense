...
def main():
    ...
    # Experiment params
    parser.add_argument('--mode', type=str, help='train or test mode', choices=['train', 'test'])
    ...
    # Model params
    parser.add_argument('--model', type=str, default='roberta-large', help='transformer model (e.g. roberta-base)')
    ...
    parser.add_argument('--ckpt', nargs="+", help='path to model checkpoint .pth file')

    # Data params
    parser.add_argument('--pred_file', type=str, help='address of prediction csv file, for "test" mode',
                        default='results.csv')
    parser.add_argument('--dataset', type=str, default='com2sense', help='list of datasets seperated by commas')
    parser.add_argument('--prompt', type=str, default='Is the following answer correct? ', help='the prompt')
    parser.add_argument('--prompt_pos', type=str, default='head', help='options: {head, tail}')
    ...
    if args.mode == 'train':
        ...
        dataset = BaseDataset('train', tokenizer=args.model, max_seq_len=args.seq_len,
                              text2text=text2text, uniqa=uniqa, prompt=args.prompt, promt_pos=args.prompt_pos)
            ...
            for batch in tqdm(train_loader):
                ...
                if curr_step % args.log_interval == 0:
                    ...
                    if val_datasets:
                        ...
                        log_msg = 'Validation Accuracy: {:.2f} %  || Validation Loss: {:.4f} || Confusion Matrix: {}\n'.format(
                            val_metrics['accuracy'], val_metrics['loss'], val_metrics['confusion_matrix'])
        ...
        assert args.ckpt is not None, "Must load trained model!\n"
        # Dataloader
        ...
        assert len(args.ckpt) == 3, "Must include 3 models\n"
        category_pred_list = []
        category_gt_list = []
        for i in range(3):
            checkpoint = torch.load(args.ckpt[i], map_location=device)
            model.load_state_dict(checkpoint['model_state_dict'])
        ...
        # Inference
            metrics = compute_eval_metrics(model, loader, device, data_len, tokenizer, text2text,
                                           is_pairwise=is_pairwise, is_test=True, parallel=args.data_parallel)
            category_pred_list.append(metrics['meta']['prediction'])
            category_gt_list.append(metrics['meta']['ground_truth'])
        ...
        acc = category_acc(category_pred_list, category_gt_list)
        print(f'Accuracy on all categories {acc}')
