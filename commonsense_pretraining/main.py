...
def main():
    ...
    if args.mode == 'train':
        ...
        for epoch in range(start_epoch, start_epoch + n_epochs):
            for batch in tqdm(train_loader):
                ...
                if curr_step % args.log_interval == 0 or curr_step == 1:
                    ...
                    if val_dataset:
                        val_metrics = compute_eval_metrics(model, val_loader, device,
                            val_used_size, tokenizer,text2text, parallel = args.data_parallel)
        ...
        test_metrics = compute_eval_metrics(model, test_loader, device,
            test_dataset.__len__(), tokenizer,text2text, parallel = args.data_parallel)
