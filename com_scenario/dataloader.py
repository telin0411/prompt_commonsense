...
class Com2SenseDataset(BaseDataset):
    ...
    def _preprocess(self, data_dir):
        ...
        # Map category labels
        label2int = {'causal': 1, 'comparison': 0}
        data = []
        if self.split == 'train' or 'dev':
            ...
            for pair in raw_data:
                sample_1 = dict(_id=pair['_id'], text=pair['sent_1'], label=pair['scenario'])
                sample_2 = dict(_id=pair['_id'], text=pair['sent_2'], label=pair['scenario'])
