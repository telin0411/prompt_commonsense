...
@torch.no_grad()
def compute_eval_metrics(prompts,batch_size,seq_len,model, dataloader, device, size, tokenizer, text2text = False, is_pairwise=False, is_test=False, parallel = False):
    """
    ...
    def decode(token_ids):   #将整数token映射回自然语言
    ...
    for batch in tqdm(dataloader):
        ...
        embeddings=torch.zeros(embeds.shape[0],seq_len+16,1024).to(device)
        atten=torch.zeros(embeds.shape[0],seq_len+16,dtype=int).to(device)
        zero=torch.zeros(1024).to(device)

        for num_emb in range(embeds.shape[0]):
            #
            temp=embeds[num_emb]    #[L,D]
            temp_att=batch['attn_mask'][num_emb]    #[L]
     #               embedding=embeddings[num_emb]   #[L+16 , D]
      #              att=atten[num_emb]     #[L+16]
            for ii in range(seq_len+16):
                #
                if ii==0:
                    #
                    ...
                    atten[num_emb][ii]=torch.tensor(1).to(device)
                elif 26<=ii<=seq_len+10:
                    embeddings[num_emb][ii]=temp[ii-11]
                    atten[num_emb][ii]=temp_att[ii-11]
                else:
                    embeddings[num_emb][ii]=zero+prompts[ii-seq_len]
                    atten[num_emb][ii]=torch.tensor(1).to(device)
        ...
        # Others
            ...
            # Forward Pass
            outs = model(atten.to(device),embeddings.to(device))
            label_logits=torch.zeros(embeds.shape[0],2).to(device)
